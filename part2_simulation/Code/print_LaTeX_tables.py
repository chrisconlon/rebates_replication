...
from table_helpers import process_main, compress_table, main_table, eff_table,table9_both,\
 effort_table, net_effects, net_effects2, merger_table, ref_table, write_tex_table

from table_helpers import compress_table2, main_table2
from table_helpers import obs_table
...
# Table 8: FC = 10;MC = 0.15
df=process_main(fn_main)
b = main_table2(compress_table2(df))
write_tex_table(table_dir / 'table_8.tex', b)

## Table 8: Observed Policy Change  137--> 144
b = obs_table(fn_main)
...
# Table A10: FC = 10;MC = 0.15 IQR arrival
df=process_main(fn_iqr)
b = main_table(compress_table(df))
write_tex_table(table_dir / 'table_A10.tex',b) 
    
# Table A11: FC = 5;MC = 0:15
df=process_main(fn_fc5)
a=main_table(compress_table(df))
write_tex_table(table_dir / 'table_A11.tex',a)
    
# Table A12: FC = 15;MC = 0.15
df=process_main(fn_fc15)
b = main_table(compress_table(df))
write_tex_table(table_dir / 'table_A12.tex',b)

# Table A13: FC = 10;MC = 0:15
df=process_main(fn_cs)
a=main_table(compress_table(df))
write_tex_table(table_dir / 'table_A13.tex',a)
